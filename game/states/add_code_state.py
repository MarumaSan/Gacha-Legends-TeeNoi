...
class AddCodeState(GameState):
    ...
    def enter(self):
        ...
        try:
            ...
            self.font_large = self.assets.load_font('assets/fonts/Monocraft.ttf', 32)  # สำหรับแสดงรางวัล (ลดจาก 48)
        ...
        try:
            self.code_frame = self.assets.load_image('assets/ui/frame code.png')
        except Exception as e:
    ...
    def on_done_click(self):
        ...
        success, message, coins = self.code_manager.redeem_code(self.code_text)
        ...
        if success:
            ...
            # บันทึกข้อมูล
            self.game.save_game()
            print(f"Code redeemed successfully! New balance: {self.game.player_data.get_coin_balance()}")
        else:
